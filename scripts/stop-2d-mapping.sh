#!/usr/bin/env bash
# Stop only processes created by start-2d-mapping.sh, then verify termination.
set -euo pipefail
...
if [[ -f "$RUNTIME_DIR/2d-mapping.pid" ]]; then
  stop_container_group "$(<"$RUNTIME_DIR/2d-mapping.pid")"
  rm -f "$RUNTIME_DIR/2d-mapping.pid"
fi

if [[ "$QUIET" != "--quiet" ]]; then
  echo "Tracked 2D mapping and camera viewer processes have been stopped."
